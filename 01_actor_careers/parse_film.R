parse_film = function(url_extension) {
  ...
  film_details = tryCatch({
    
      output = list()

      page_raw = glue("https://www.rottentomatoes.com/{url_extension}") %>%
      read_html()
  ...
  # Need to replace NULLS with NA's or else it ruins the list -> rbind -> data frame
  film_details = lapply(film_details, function(x){ if(length(x)==0) {NA} else {x}})
