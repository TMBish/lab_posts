...
chart_data = 
  ...
  group_by(role_type) %>%
  mutate(
    films_prop = films / sum(films),
    people_prop = distinct_people / sum(distinct_people)
  ) %>% ungroup()



map(c("Actor", "Director", "Director (Non Top250)"), function(x) {

    chart_data %>%
    filter(role_type == x) %>%
    gather("variable", "value", films_prop, people_prop) %>%
    hchart(hcaes(x = gender, group = variable, y = value)) %>%
    hc_title(text = x)
