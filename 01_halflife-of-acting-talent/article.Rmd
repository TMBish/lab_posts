...
dict %>% pandoc.table(style = "rmarkdown")
...
films %>% 
  ...
  pandoc.table(emphasize.strong.cols = 3, missing="?", style = "rmarkdown", justify="left",split.cells=15)
...
genders %>%
  ...
  pandoc.table(emphasize.strong.cols = 3, style = "rmarkdown", justify="left", split.cells=15)
```
...
Well, as it turns out, it depends on which one of the above groups you're in. To remind you, our groups are: directors (predominantly male as seen above), male actors, and female actors. For all these individuals we calculate their age at the time they were involved in a realeased film and plot the distribution within each group.  
...
There's a few other things we want to control for here. I don't necessarily want to plot raw film critical rating because this might be prone to idiosyncracies within groups or ages - for example, high performing directors might generally wait until they feel experienced enough to direct their first film.

Instead we'll plot, for each individual and film, how well they have perfomed relative to that individual's career average (the average tomatometer of all that individual's films). To illustrate this approach let's look at Denzel Washington's films. Denzel has a healthy mean tomatometer of 68%.
...
master %>% 
  ...
  pandoc.table(emphasize.strong.cols = 6, style = "rmarkdown", justify="left")

```

So based on this 10 film sample Denzel had a solid early 40s where his films (of the likes **Hurricane** and **Courage Under Fire**) average a rotten tomatoes rating about 20% above his career average. He looks like he dropped off after 50 with a few flops like **Déjà Vu** (which I didn't mind) and **2 Guns**. Look I'm not calling him washed but he hasn't done anything good for a while.

## Prime Suspects

To help visualise this approach let's take 12 of the top 20 actors by volume and visualise average their film rating performance by age. De Niro is a great example of someone who starred in increasingly poor films as he got older - clearly visible in the chart below.. Obviously this probably has more to do with role selection than deteriorating skills (we'll discuss this later) but it's interesting to visualise nonetheless.
