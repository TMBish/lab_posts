...
#the-half-list-of-acting-talent img {
  height: 200px; 
  display:block; 
  margin: 0 auto;
}
...
Let's have a look at the films data set. Click the blue `+` button to view the extra fields in the dataset (the director and top 6 actors in the film).
...
So generally this is looking pretty, pretty good. Things get dicey with unisex names like Sandy; however, the package must know that most Sandys born in 1937 were female so we'll have to trust it here.
...
gender_prop_finding = create_finding("Approximately 80% of all directors are male, and 85% of popular films are directed by a man.")
...
So we'll treat directors as a single group that, statistically speaking, are very much likely to be males.
...
age_dist_chart = 
  ...
  hc_tooltip(
    formatter = JS("function(){return('<b> Series: </b>' + point.key + '<br> <b> Age: </b>' + this.x + '<b> Value: </b>' + this.y )}")
  )
...
There's a few other things we want to control for here. I don't necessarily want to plot raw film critical rating because this might be prone to idiosyncracies within groups or ages - for example, high performing directors might generally wait until they are experienced enough to direct their first film.

Instead we'll plot, for each individual and film, how well they have perfomed relative to their career average (the average tomatometer of all that individual's films). To illustrate this approach let's look at Denzel Washington's films. Denzel has a healthy mean tomatometer of 68%.
...
So based on this 10 film sample Denzel had a solid early 40s where his films (**Hurricane** and **Courage Under Fire** etc) averaged a rotten tomatoes rating about 20% above his career average. He looks like he dropped off after 50 with a few flops like **Déjà Vu** (which I didn't mind) and **2 Guns**. Look I'm not calling him washed but he hasn't done anything good for a while.
...
}) %>% hw_grid(ncol = 3, rowheight = 350) %>% browsable()
...
So let's extend this approach and plot out the median tomato index by age at production, giving us an idea of the *average performance above or below a career average* at each age.
...
While it appears that actors and directors have their prime between the ages of 27-35 I don't think it's fair to say this is **caused** by a natural skill trajectory. Some caveats that could potentially explain this trend without our skills and abilities narrative:

* Most films typically have non-elderly main characters; limiting the ability for an older, but still skillful, actor to be cast in these roles
* Seperating an Actor's role **choice** vs their **performance** given they chose a particular role highlights the problem we see with De Niro above. It's quite possible that after career success and achievements actor's and directors prioritise different things (eg. *$Cash Money$*) in the twiglights of their careers.
